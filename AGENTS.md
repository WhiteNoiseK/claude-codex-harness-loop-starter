...
> **This section applies to ALL engines** (Claude, Codex).
> When an engine starts any task, it must read §12 first and comply unconditionally.
> Engine-specific detail file: `CODEX.md` (Codex raw exec constraints).
...
When Claude delegates any implementation task to Codex, the handoff prompt **must** open with:
...
Omitting this header from a raw `codex exec` call is an operator error — not a model error.
