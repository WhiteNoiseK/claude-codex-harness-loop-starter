...
**Multi-engine model** — the harness runs across two engines; each has a fixed role:
...
When Claude delegates a task to Codex, the prompt must begin with the
...
> Before delegating any stage to Codex, prepend the "WRITE SCOPE IS CLOSED" header (`AGENTS.md §12.3`).
